...
#     pass
    ...
    W2 = self.params['W2']
    b2 = self.params['b2']
#     scores = np.maxmum(X.dot(W1)+b1)*W2 + b2
    # 其实可以像上述注释那样直接写完，但是分开来写的话，会方便后面计算gradient。可能是我比较菜的原因要分开写
...
#     pass
    ...
    loss += -scores[np.arange(N),y].sum() + np.log(margins).sum()
    loss /= N
    loss += 0.5*self.reg*(np.sum(W1*W1) + np.sum(W2*W2))
    
    dL = exp_scores / margins
    dL[np.arange(N),y] += -1  #shape (N,C)
    dL /= N
    grads['b2'] = np.sum(dL,axis=0)
    grads['W2'] = Relu.T.dot(dL) + self.reg * W2 # Relu.shape = (N,hidden_dims)  W2.shape = (hidden_dims,C)
    dRelu = dL.dot(W2.T) # shape 一定要符合矩阵点乘的标准
    dRelu[(X.dot(W1)+b1)<0] = 0.0  # dRelu.shape = (N,hidden_dims)
    grads['b1'] = np.sum(dRelu,axis=0)
    grads['W1'] = X.T.dot(dRelu) + self.reg * W1
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.use_dropout = dropout > 0 # original
#     self.use_dropout = dropout>=0 # change to it
    self.reg = reg
    ...
    ############################################################################
#     pass
#     self.params['W1'] = np.random.norm(0,weight_scale,[input_dim, hidden_dims])
#     self.params['b1'] = np.zeros((hidden_dims,))
#     self.params['W2'] = np.random.norm(0,weight_scale,[hidden_dims,num_classes])
#     self.params['b2'] = np.zeros((num_classes,))
