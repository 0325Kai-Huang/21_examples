...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    # mul = np.multiply(np.dot(X, self.X_train.T), -2) # 500*3072 dot 3072*5000 = (500,5000)
  ...
  def getNormMatrix(self, x, lines_num):
    ...
    """ 
    # print((np.ones((lines_num, 1)) * np.sum(np.square(x), axis = 1)).shape)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      #########################################################################
      # 上面closest_y = [] 这句话是初始化list 但是list 不能用[i,:]这种切片方式，会报错
      # TypeError: list indices must be integers or slices, not tuple
      # 因此我尝试将 closest_y 初始化为 全0 的 np.array类型，这样就可以用[i,:]
      # 但是如何初始化这个维度？  closest_y 是用来获取 k 个距离最小的索引的。所以将列数做为k，行数为num_test
      closest_y = np.zeros((num_test,k),dtype = 'int64')
